...
public Plugin:myinfo = {
	...
	author = "BomBom - Dunceantix, Medowar",
	description = "FragRadio SourceMod Plugin, edited by Medowar(germangang.com)",
	version = PLUGINVERSION,
	url = "http://fragradio.co.uk/"
}
...
public OnPluginStart() {
	...
	g_hCvarInteract = CreateConVar("fr_interact", "0", "Enables or disables the request and shoutout system.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(g_hCvarInteract, Cvar_Changed);
	
	g_hCvarRating = CreateConVar("fr_rating", "0", "Enables or disables the song and dj rating system.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	g_hCvarAdverts = CreateConVar("fr_adverts", "0", "Enables or disables chat adverts that display after a time limit.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(g_hCvarAdverts, Cvar_Change_Adverts);
	
	g_hCvarAdvertsInterval = CreateConVar("fr_adverts_interval", "180.0", "Sets the delay between adverts shown in chat.", FCVAR_PLUGIN);
	...
	g_hCvarStreamInterval = CreateConVar("fr_streaminfo_interval", "1.0", "Sets the time between stream info updates.", FCVAR_PLUGIN);
	HookConVarChange(g_hCvarStreamInterval, Cvar_Changed);

	g_hCvarStreamHost = CreateConVar("fr_stream_host", "DOMAIN ADDRESS IN HERE", "Sets the website hostname used to send and retrieve info.", FCVAR_PLUGIN);
	...
	g_hCvarStreamGamePath = CreateConVar("fr_stream_game_path", "FILE1.php", "Website path used for rating and interaction.", FCVAR_PLUGIN);
	HookConVarChange(g_hCvarStreamGamePath, Cvar_Changed);
	
	g_hCvarStreamUpdatePath = CreateConVar("fr_stream_update_path", "FILE2.php", "Website path used for sending server info.", FCVAR_PLUGIN);
	HookConVarChange(g_hCvarStreamUpdatePath, Cvar_Changed);
	
	g_hCvarStreamStatsPath = CreateConVar("fr_stream_stats_path", "FILE3.php", "Website path used for getting stream info.", FCVAR_PLUGIN);
	HookConVarChange(g_hCvarStreamStatsPath, Cvar_Changed);
	
	g_hCvarWebPlayerScript = CreateConVar("fr_webplayer_script", "FILE4.php", "Website script used for the web player.", FCVAR_PLUGIN);
	HookConVarChange(g_hCvarWebPlayerScript, Cvar_Changed);
	
	g_hCvarServerUpdateInterval = CreateConVar("fr_server_update_interval", "1.0", "Sets the time between server info updates.", FCVAR_PLUGIN);
...
public Action:Cmd_RadioMenu(client, args) {
	...
	SetMenuTitle(menu, "FragRadio.co.uk - Menu\n \nDJ: %s\nSong: %s\n \n", g_sDJ, g_sSong);
	
	AddMenuItem(menu, "100", "100% Volume");
	AddMenuItem(menu, "80", "80% Volume");
	AddMenuItem(menu, "60", "60% Volume");
